...
const remove_href = () => {
	let indexes = []
	$('.lesson-links .icon-sm use').filter(function() {
		return $(this).attr('href') === '#icon-lock';
	}).each(function() {
		var index = $('.lesson-links .icon-sm use').index(this);
		indexes.push(index)
	});
	all_lessons = $('.lesson-links')
	indexes.forEach((val)=>{
		row_lesson = $(all_lessons[val]).attr('href','#')
	})
};
