...
const add_web_link = (frm) => frm.add_web_link(`/courses/${frm.doc.name}`, "See on Website");

const set_currency = (frm) => {
	if (!frm.doc.currency)
		frappe.db.get_single_value("LMS Settings", "default_currency").then((value) => {
	...
	frappe.confirm(
		...
		() => {
			frappe.call({
				...
				callback: function (r) {
					if (r.message == "OK") {
						frappe.msgprint("All members re-enrolled successfully");
					}
				},
			});
		},
	...
	dialog.set_primary_action(__("Create"), function () {
		...
		frappe.call({
			...
			callback: function (r) {
				dialog.hide();
				if (r.message == "OK") {
					frappe.msgprint("Member re-enrolled successfully");
				}
			},
		});
	});
