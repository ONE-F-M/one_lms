			...
			>
				...
				>
					...
					<div class="flex items-center space-x-2 my-4">
						...
						>
							<template #prefix>
								<Pause class="size-4 stroke-1.5" />
							</template>
							{{ __("Pause") }}
						</Button>

						<Button v-if="isReading" @click="stopReadAloud" variant="secondary">
							<template #prefix>
								<Square class="size-4 stroke-1.5" />
							</template>
							{{ __("Stop") }}
						</Button>
					</div>
...
<script setup>
import { Badge, Breadcrumbs, Button, call, createResource, Tooltip, usePageMeta } from "frappe-ui";
...
watch(
	...
	async ([newChapterNumber, newLessonNumber], [oldChapterNumber, oldLessonNumber]) => {
...
const toggleReadAloud = () => {
	...
	// Get the text from the lesson content area
	const text = lessonContentRef.value.innerText;
	...
	if (!isReading.value || isPaused.value) {
		...
		// Resume if paused, otherwise start new speech
		if (isPaused.value) {
...
const setupLesson = (data) => {
	...
	if (data.instructor_content && JSON.parse(data.instructor_content)?.blocks?.length > 1)
...
const renderEditor = (holder, content) => {
	// empty the holder
	if (document.getElementById(holder)) document.getElementById(holder).innerHTML = "";
	return new EditorJS({
		holder: holder,
...
const switchLesson = (direction) => {
	trackVideoWatchDuration();
	let lessonIndex =
		direction === "prev" ? lesson.data.prev.split(".") : lesson.data.next.split(".");
...
const checkIfDiscussionsAllowed = () => {
	...
	if (
		...
		(lesson.data?.membership || user.data?.is_moderator || user.data?.is_instructor)
	)
		allowDiscussions.value = true;
};
...
.plyr__control--overlaid {
	background: radial-gradient(circle, rgba(0, 0, 0, 0.4) 0%, rgba(0, 0, 0, 0.5) 50%);
}
